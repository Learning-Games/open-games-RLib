...
import qualified Engine.QLearning.Export as QLearningCalvanoReplication
...
main = do
  iters <- fmap (fromMaybe [10000, 20000] . (>>=readMaybe)) (lookupEnv "ITERS")
  gEnv1   <- newStdGen
  gEnv2   <- newStdGen
  gObs1   <- newStdGen
  gObs2   <- newStdGen
  defaultMain
          ...
          (nfIO
             ( let
               -- Number of runs to be executed
                ...
                -- Game parameters
                parametersGame :: StdGen -> StdGen -> StdGen -> StdGen -> StdGen -> StdGen -> CalvanoReplication.Parameters
                parametersGame gEnv1 gEnv2 gPrice1 gPrice2 gObs1 gObs2 = CalvanoReplication.Parameters
                  { pKsi = 0.1
                  , pBeta =  0.000004
                  , pBertrandPrice = 1.47
                  , pMonopolyPrice = 1.92
                ...
                exportConfigGame name parameters = QLearningCalvanoReplication.ExportConfig
                  { iterations =  10
                  -- ^ how many iterations?
                  ...
                  , mapStagesM_ = CalvanoReplication.mapStagesM_ parameters
                  , initial = CalvanoReplication.initialStrat parameters >>= CalvanoReplication.sequenceL
                  , ctable = CalvanoReplication.actionSpace parameters
                  , mkObservation = \a b -> CalvanoReplication.Obs (a, b)
                  , runName = name
                  , players = 2
                  }
                ...
                specification name = do
                  ...
                  BS.writeFile (fromRelDir  (dirResultIteration </> parametersFile)) (CalvanoReplication.csvParameters parameters)
                  QLearningCalvanoReplication.runQLearningExportingDiagnostics exportConfig
                  runProcess_
                    (proc
                        "touch"
                  ...
                  runProcess_
                    (shell
                        ( "tail "
                        ++ "-"
                        ++ (show $  keepOnlyNLastIterations * QLearningCalvanoReplication.players exportConfig)
                        ++ " "
    ...
    , bgroup
        "AsymmetricLearnersSingleExperiment"
        [bench
          ("iters/" ++ show i)
          (nfIO
             (
              ...
              executeAndRematchSingleRun name = do
                expLowC  <- firstStageLearning name parametersGameLowC exportConfigGameLearning
                -- ^ Instantiate run with low costs
                -- expHighC <- firstStageLearning name parametersGameHighC exportConfigGameLearning -- Just a single experiment for comparison
                -- ^ Instantiate run with high costs
                rematchedLearning name expLowC expLowC
                -- ^ Rematching
        ...
        [bench
          ...
          (nfIO
             ( let 
              ...
              -- Player parameters for learning phase
              -- Game with low costs
              parametersGameLowC :: StdGen -> StdGen -> StdGen -> StdGen -> AsymmetricLearners.Parameters
              parametersGameLowC gEnv1 gEnv2 gObs1 gObs2 = AsymmetricLearners.Parameters
                { pKsi = 0.1
                , pBeta =  0.000004
                , pInitialExploreRate1 = ((exp 1) ** 0)
                , pInitialExploreRate2 = ((exp 1) ** 0)
              ...
              -- Game with high costs
              parametersGameHighC :: StdGen -> StdGen -> StdGen -> StdGen -> AsymmetricLearners.Parameters
              parametersGameHighC gEnv1 gEnv2 gObs1 gObs2 = AsymmetricLearners.Parameters
                { pKsi = 0.1
                , pBeta =  0.000004
                , pInitialExploreRate1 = ((exp 1) ** 0)
                , pInitialExploreRate2 = ((exp 1) ** 0)
              ...
              -- Parameters after learning took place
              -- player1 of experiment 1 is matched with player 2 experiment 2
              parametersGameRematchingP1E1P2E2 :: StdGen -> StdGen -> StdGen -> StdGen -> Q.ExploreRate -> Q.ExploreRate -> AsymmetricLearners.Parameters
              parametersGameRematchingP1E1P2E2 gEnv1 gEnv2 gObs1 gObs2 exploreRate1 exploreRate2 = AsymmetricLearners.Parameters
                { pKsi = 0.1
                , pBeta =  0.000004
                , pInitialExploreRate1 = exploreRate1
                , pInitialExploreRate2 = exploreRate2
              ...
              parametersGameRematching :: StdGen -> StdGen -> StdGen -> StdGen -> Q.ExploreRate -> Q.ExploreRate -> AsymmetricLearners.Parameters
              parametersGameRematching gEnv1 gEnv2 gObs1 gObs2 exploreRate1 exploreRate2 = AsymmetricLearners.Parameters
                { pKsi = 0.1
                , pBeta =  0.000004
                , pInitialExploreRate1 = exploreRate1
                , pInitialExploreRate2 = exploreRate2
              ...
              -- Configuration of run and export parameters for initial learning run
              exportConfigGameLearning name parameters = ExportAsymmetric.ExportConfig
                { iterations = i
                -- ^ how many iterations?
                , incrementalMode = True
                -- ^ report incremental changes to qmatrix or export full qmatrix with each iteration?
                ...
                -- ^ For complete reporting of Q-values, how often should values be exported?
                  , threshold = 100000
                -- ^ Stopping criterion: After how many runs should the computation be stopped?
                , mapStagesM_ = AsymmetricLearners.mapStagesM_ parameters
                , initial = AsymmetricLearners.initialStrat parameters (AsymmetricLearners.initialArray1 parameters) (AsymmetricLearners.initialArray2 parameters)  >>= AsymmetricLearners.sequenceL
                , ctable1 = AsymmetricLearners.actionSpace1 parameters
                , ctable2 = AsymmetricLearners.actionSpace2 parameters
                , mkObservation = \a b -> AsymmetricLearners.Obs (a, b)
                , runName = name
                , players = 2
                }
              ...
              exportConfigGameRematching name parameters arr1 arr2 = ExportAsymmetric.ExportConfig
                      ...
                      -- ^ Stopping criterion: After how many runs should the computation be stopped?
                      , mapStagesM_ = AsymmetricLearners.mapStagesM_ parameters
                      , initial = AsymmetricLearners.initialStrat parameters arr1 arr2 >>= AsymmetricLearners.sequenceL
                      , ctable1 = AsymmetricLearners.actionSpace1 parameters
                      , ctable2 = AsymmetricLearners.actionSpace2 parameters
                      , mkObservation = \a b -> AsymmetricLearners.Obs (a, b)
                      , runName = name
