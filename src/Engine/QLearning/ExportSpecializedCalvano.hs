...
module Engine.QLearning.ExportSpecializedCalvano
...
import          Examples.QLearning.CalvanoReplicationFixedParameters (Observation(..), PriceSpace(..))
...
data RewardDiagnostics = RewardDiagnostics
  { iteration, player, state_action_index, actionNew, observation1, observation2  :: !Int
...
instance BuildCsvRow RewardDiagnostics where
  buildCsvRow RewardDiagnostics{iteration,player,state_action_index,action_choice,max,actionNew, observation1,observation2,prediction,explore_rate,reward} =
    ...
    SB.intDec actionNew <> "," <>
    SB.intDec observation1 <> "," <>
    SB.intDec observation2 <> "," <>
...
instance BuildHeaders RewardDiagnostics where
  buildHeaders _ = "iteration,player,state_action_index,action_choice,max,actionNew,observation1, observation2, prediction,explore_rate,reward"
  {-# INLINE buildHeaders #-}
                            ...
                            RewardDiagnosticsMsg QLearning.RewardDiagnostics {..} -> do
                              writeRewardExtendedRow
                                RewardDiagnostics
                                  ...
                                  , observation1  = QLearning.unInt $ QLearning.toIdx $ fst $ helperObs rewardDiagObservation
                                  , observation2  = QLearning.unInt $ QLearning.toIdx $ snd $ helperObs rewardDiagObservation
...
writeStateActionIndex ExportConfig {..} initial' = do
  ...
  putStrLn $ BSU.fromString $ show initial'
...
writeQValues ExportConfig {..} maximalState initial'@(p1_0 ::- p2_0 ::- Nil) writeRow = do
  ...
  mapStagesM_
    (\State {prev, iteration, skipping} (p1 ::- p2 ::- Nil) -> do
       ...
       when (save /= prev) (do sequence_
                                [ (putStrLn (toFixed 2 percent <> "% complete ..."))
                                , (putStrLn $ BSU.fromString $ show p1 )
                                , (putStrLn $ BSU.fromString $ show p2)])
       let
        in when
