...
module Engine.QLearning.ImportAsymmetricLearnersOwnPrice
...
type Action = Double
type Value  = Double
type QValueMatrixTarget = (Action,Action, Value)
...
parseStateActionRow = do
  state <- many $ digitChar <|> char '.'
  _      <- mySeparator
  action <- many $ digitChar <|> char '.'
  _      <- mySeparator
  index  <- many $ digitChar <|> char '.'
  return $ StateActionInput (read state) (read action) (read index)
...
toStateActionQValues vStateAction vQValues =
  ...
  where
     lsStateActions = [(st,ac,i)| (StateActionInput st ac i) <- V.toList vStateAction]
     lsQValues      = [(i,qv) | (QValueRow _ _ i qv) <- V.toList vQValues]

-- Merges the two lists into the target for the strategies
transformIndexList :: [(Action,Action,Int)] -> [(Int,Value)] -> [QValueMatrixTarget]
transformIndexList [] _  = []
transformIndexList _  [] = []
transformIndexList ls ((i,v):ivs) = case (findElement ls i) of
  Left _ -> transformIndexList ls ivs
  Right (s,a1) -> (s,a1,v) : transformIndexList ls ivs

findElement :: [(Action,Action,Int)] -> Int -> Either String (Action,Action)
findElement [] i = Left $ F.formatToString ("Element " F.% F.int F.% " was not found") i 
findElement ((s,a1,i1):xs) i =
  if  i == i1 then  Right (s,a1)
