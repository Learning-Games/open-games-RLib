...
-- | Contains the same as Engine.QLearningVerboseOutput.ExportAsymmetricLearnersLogReduced but uses the VerboseOutput version of the QlLearning for inspecting and debugging

module Engine.QLearning.ExportAsymmetricLearnersLogReducedVerboseOutput
...
import           Engine.QLearningVerboseOutput (ToIdx, toIdx, Idx, QLearningMsg(..), Env, CTable(..),ActionChoice)
import qualified Engine.QLearningVerboseOutput as QLearningVerboseOutput
import           Engine.TLL
...
data RewardDiagnostics = RewardDiagnostics
  ...
  , explore_rate :: !QLearningVerboseOutput.ExploreRate
  , reward :: !Double
  }
...
data ExportConfig n o a m = ExportConfig
  ...
  , mapStagesM_ ::
    forall s.
       (s -> List '[ (a, Env n o a), (a, Env n o a)] -> m (QLearningVerboseOutput.Decision s))
                  ...
                  (RIO.mkGLogFunc
                      (\_backtrace msg ->
                        case msg of
                          ...
                          RewardDiagnosticsMsg QLearningVerboseOutput.RewardDiagnostics {..} -> do
                            writeRewardExtendedRow
                              RewardDiagnostics
                          ...
                          QTableDirtied QLearningVerboseOutput.Dirtied {..} ->
                            when
  ...
  when
    ...
    (do dirResultIteration <- parseRelDir runName
        withCsvFile
          ...
          (\writeRow -> do
            let (_, env) ::- _ = initial'
            bounds' <- liftIO (A.getBounds (QLearningVerboseOutput._qTable env))
            liftIO
              (V.sequence_
                  [ writeRow
...
writeQValues ExportConfig {..} maximalState initial'@(p1_0 ::- p2_0 ::- Nil) writeRow = do
  ...
  mapStagesM_
    (\State {prev, iteration, skipping} (p1 ::- p2 ::- Nil) -> do
       ...
       pure
         (if terminate
            then QLearningVerboseOutput.Stop
            else QLearningVerboseOutput.Continue
                   (State
                      { prev = save
    ...
    writePlayerQTable iteration player (_, env) = do
      liftIO
        (mapWithIndex_
           ...
           (QLearningVerboseOutput._qTable env))
...
updateMaximalTables ::
  ...
  => Int
  -> MaximalState n o a
  -> QLearningVerboseOutput.MaximalAction n o a
  -> Int
  -> IO ()
updateMaximalTables threshold MaximalState {..} QLearningVerboseOutput.MaximalAction {..} playerCount = do
  !tbl <-
    fmap
      (HM.insertWith
