...
type IOOpenGame a b x s y r = OpenGame MonadOpticMarkov MonadContextMarkov a b x s y r
...
dependentDecisionIO name sampleSize ys = OpenGame { play, evaluate} where
  ...
  play = \(strat ::- Nil) -> let v x = do
                             ...
                             in MonadOpticMarkov v u

  evaluate (strat ::- Nil) (MonadContextMarkov h k) =
    output ::- Nil
...
fromLens v u = OpenGame {
  play = \Nil -> MonadOpticMarkov (\x -> return (x, v x)) (\x r -> return (u x r)),
  evaluate = \Nil _ -> Nil}
