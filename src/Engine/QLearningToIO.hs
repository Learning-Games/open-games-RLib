...
module Engine.QLearningToIO where
...
-- policies and a simple IO based equilibrium check as well as a
-- connector to the Monte Carlo simulation framework in the future
---------------------------------------------------------------------
...
fromLens :: Monad m => (x -> y) -> (x -> r -> s) -> InteractiveMonadicStageGame m '[] '[] x s y r
fromLens v u = OpenGame {
  play = \Nil -> MonadOptic (\x -> pure (x, v x)) (\x -> (\r -> pure $ u x r)),
  evaluate = \Nil _ -> Nil}


fromFunctions :: Monad m => (x -> y) -> (r -> s) -> InteractiveMonadicStageGame m '[] '[] x s y r
