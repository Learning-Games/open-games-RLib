...
-------------------------------------------------------------
-- 6 Implementation for Markov games

data MonadOpticMarkov s t a b where
  MonadOpticMarkov :: (s -> IO (z, a))
                          -> (z -> b -> StateT Vector IO t)
                          -> MonadOpticMarkov s t a b

instance Optic MonadOpticMarkov where
  lens v u = MonadOpticMarkov (\s -> return (s, v s)) (\s b -> return (u s b))
  (>>>>) (MonadOpticMarkov v1 u1) (MonadOpticMarkov v2 u2) = MonadOpticMarkov v u
    where v s = do {(z1, a) <- v1 s; (z2, p) <- v2 a; return ((z1, z2), p)}
          u (z1, z2) q = do {b <- u2 z2 q; u1 z1 b}
  (&&&&) (MonadOpticMarkov v1 u1) (MonadOpticMarkov v2 u2) = MonadOpticMarkov v u
    where v (s1, s2) = do {(z1, a1) <- v1 s1; (z2, a2) <- v2 s2; return ((z1, z2), (a1, a2))}
...
data MonadContextMarkov s t a b where
  MonadContextMarkov :: (Show z) => IO (z, s) -> (z -> a -> StateT Vector IO b) -> MonadContextMarkov s t a b

instance Precontext MonadContextMarkov where
  void = MonadContextMarkov (return ((), ())) (\() () -> return ())

instance Context MonadContextMarkov MonadOpticMarkov where
  cmap (MonadOpticMarkov v1 u1) (MonadOpticMarkov v2 u2) (MonadContextMarkov h k)
            = let h' = do {(z, s) <- h; (_, s') <- v1 s; return (z, s')}
                  k' z a = do {(z', a') <- lift (v2 a); b' <- k z a'; u2 z' b'}
               in MonadContextMarkov h' k'
  (//) (MonadOpticMarkov v u) (MonadContextMarkov h k)
            = let h' = do {(z, (s1, s2)) <- h; return ((z, s1), s2)}
                  k' (z, s1) a2 = do {(_, a1) <- lift (v s1); (_, b2) <- k z (a1, a2); return b2}
               in MonadContextMarkov h' k'
  (\\) (MonadOpticMarkov v u) (MonadContextMarkov h k)
