...
module Engine.QLearningSimple where
...
deriving instance (Show a, Show (o a), Show (o (Idx a)), Memory n) => Show (State n o a)
-- ^ Added here the agent observation the idea is that global and local information might diverge
instance (NFData a, NFData (o a), NFData (o (Idx a)), (NFData (Memory.Vector n (o (Idx a))))) => NFData (State n o a)
...
updateTemperature decreaseFactor =  temperature %~ (* decreaseFactor)

-- Update explorRate
updateExploreRate :: ExploreRate -> State n o a -> State n o a
updateExploreRate decreaseFactor =  exploreRate %~ (* decreaseFactor)
...
chooseActionNoExplore support s = do
  maxed <- liftIO $ maxScore obsVec (_qTable s) support
  let (exploreR, gen') = Rand.randomR (0.0 :: Double, 1.0 :: Double) (_randomGen $ s)
...
chooseExploreAction support s = do
  ...
  let (exploreR, gen') = Rand.randomR (0.0, 1.0) (_randomGen s)
  if exploreR < _exploreRate s
    then do
...
updateQTableST learningRate gamma support s obs2 action reward  = do
        ...
        let (_exp, gen')  = Rand.randomR (0.0 :: Double, 1.0 :: Double) (_randomGen s)
...
chooseLearnDecrExploreQTable learningRate gamma decreaseFactorExplore support s obs2 action reward  = do
       ...
       prediction    <- liftIO $ A.readArray table0 (obsVec, toIdx action)
       maxed <- liftIO $ maxScore (Memory.pushEnd obsVec (fmap toIdx obs2)) table0 support
       let  (_,gen')     = Rand.randomR (0.0 :: Double, 1.0 :: Double) (_randomGen s)
            updatedValue = reward + gamma * (fst $ maxed)
            newValue     = (1 - learningRate) * prediction + learningRate * updatedValue
       liftIO $ A.writeArray table0 (Memory.pushEnd obsVec (fmap toIdx (_obs s)), toIdx action) newValue
...
pureDecisionQStage :: Monad m =>
                      ...
                      -> QLearningStageGame m '[m (a,State n o a)] '[m (a,State n o a)] () () a (Double,(o a))
pureDecisionQStage actionSpace name chooseAction updateQTable = OpenGame {
  play =  \(strat ::- Nil) ->  let  v obs = do
                                           (_,s) <- strat
                                           action   <- ST.evalStateT  (chooseAction actionSpace s) s
                                           pure ((),action)
  ...
  evaluate = \(strat ::- Nil) (MonadContext h k) ->
              let
                output = do
                   (_,state') <- strat
                   (z,_) <- h
                   action <- ST.evalStateT  (chooseAction actionSpace state') state'
                   (reward,obsNew) <- k z action
                   state'' <- ST.execStateT (updateQTable actionSpace state' obsNew  action reward)
                                            state'
                   return (action,state'')
                in (output ::- Nil)}
