...
initialEnv2 = initialArray >>= \arr ->  pure $ Env "PLayer2" 2 arr 0  0.2  (Rand.mkStdGen 100) (fmap (fmap toIdx) (Memory.fromSV(SV.replicate initialObservation))) (5 * 0.999) "NothingHappendYet" 0  (fmap (fmap toIdx) (Memory.fromSV(SV.replicate initialObservation)))
