...
module Examples.QLearning.OwnPriceObservation.QLearningToBayesian where
...
fromListToStrategy ls = Kleisli $ \s -> certainly $ findOptimalAction ls s
  where
    findOptimalAction :: [(s,a,Double)] -> s -> a
    findOptimalAction ls state1 = fst $ maximumBy (comparing snd) ls'
      where
