...
module Examples.QLearning.AsymmetricLearnersNew.Internal4
  where
...
rematchedLearning :: String
                  ...
                  -> (StdGen -> StdGen -> StdGen -> StdGen -> Parameters)
                  -- ^ Parameters specific for the rematching player1 from experiment 1 and player 2 from experiment 3
                  -> (StdGen -> StdGen -> StdGen -> StdGen -> Parameters)
                  -- ^ Parameters specific for the rematching player1 from experiment 2 and player 2 from experiment 1
                  -> (StdGen -> StdGen -> StdGen -> StdGen -> Parameters)
                  -- ^ Parameters specific for the rematching player1 from experiment 2 and player 2 from experiment 3
                  -> (StdGen -> StdGen -> StdGen -> StdGen -> Parameters)
                  -- ^ Parameters specific for the rematching player1 from experiment 3 and player 2 from experiment 1
                  -> (StdGen -> StdGen -> StdGen -> StdGen -> Parameters)
                  -- ^ Parameters specific for the rematching player1 from experiment 3 and player 2 from experiment 2
                  -> (StdGen -> StdGen -> StdGen -> StdGen -> Parameters)
                  ...
                  -> (StdGen -> StdGen -> StdGen -> StdGen -> Parameters)
                  -- ^ Control matching: Parameters specific for the rematching player1 from experiment 3 and player 2 from experiment 3
                  -> (String
                      -> Parameters
                      -> IO
                            (QTable
                 ...
                 -> ((QTable Player1N Observation PriceSpace, QTable Player2N Observation PriceSpace), Observation PriceSpace)
                 -> IO ()
rematchedLearning name keepOnlyNLastIterations p1e1p2e2 p1e1p2e3 p1e2p2e1 p1e2p2e3 p1e3p2e1 p1e3p2e2 p1e1p2e1 p1e2p2e2 p1e3p2e3 exportConfigGameRematching ((x1,x2),lastObsX) ((y1,y2),lastObsY)  ((z1,z2),lastObsZ) = do
  x1'      <- copyArrayP1 x1
  x1''     <- copyArrayP1 x1
  y1'      <- copyArrayP1 y1
  y1''     <- copyArrayP1 y1
  ...
  z2'      <- copyArrayP2 z2
  z2''     <- copyArrayP2 z2
  pairing2 ("p1e1p2e2_run" ++ name) keepOnlyNLastIterations p1e1p2e2 exportConfigGameRematching (x1',y2') randomInitialObservation
  pairing2 ("p1e1p2e3_run" ++ name) keepOnlyNLastIterations p1e1p2e3 exportConfigGameRematching (x1'',z2') randomInitialObservation
  pairing2 ("p1e2p2e1_run" ++ name) keepOnlyNLastIterations p1e2p2e1 exportConfigGameRematching (y1',x2') randomInitialObservation
  pairing2 ("p1e2p2e3_run" ++ name) keepOnlyNLastIterations p1e2p2e3 exportConfigGameRematching (y1'',z2'') randomInitialObservation
  pairing2 ("p1e3p2e1_run" ++ name) keepOnlyNLastIterations p1e3p2e1 exportConfigGameRematching (z1',x2'') randomInitialObservation
  pairing2 ("p1e3p2e2_run" ++ name) keepOnlyNLastIterations p1e3p2e2 exportConfigGameRematching (z1'',y2'') randomInitialObservation
  pairing2 ("p1e1p2e1_run" ++ name) keepOnlyNLastIterations p1e1p2e1 exportConfigGameRematching (x1,x2) (\_ -> lastObsX)
  pairing2 ("p1e2p2e2_run" ++ name) keepOnlyNLastIterations p1e2p2e2 exportConfigGameRematching (y1,y2) (\_ -> lastObsY)
  pairing2 ("p1e3p2e3_run" ++ name) keepOnlyNLastIterations p1e3p2e3 exportConfigGameRematching (z1,z2) (\_ -> lastObsZ)
