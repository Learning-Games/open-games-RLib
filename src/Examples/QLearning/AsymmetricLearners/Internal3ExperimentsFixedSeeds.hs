...
module Examples.QLearning.AsymmetricLearners.Internal3ExperimentsFixedSeeds
  where
...
executeAndRematchSingleRun name parameters1 parameters2 parameters3 exportConfigGameLearning keepOnlyNLastIterations parametersGameRematchingMap exportConfigGameRematching = do
  ...
  rematchedLearning name keepOnlyNLastIterations parametersGameRematchingMap exportConfigGameRematching  expLowC expMedC expHighC
  -- ^ Rematching
...
rematchedLearning :: String
                  ...
                  -> (QTable
                          ...
                          QTable
                              Player2N Observation PriceSpace)
                  -> IO ()
rematchedLearning name keepOnlyNLastIterations parametersGameRematching exportConfigGameRematching (x1,x2) (y1,y2) (z1,z2)  = do
  x1'      <- copyArray x1
  x1''     <- copyArray x1
  y1'      <- copyArray y1
  y1''     <- copyArray y1
  ...
  z2'      <- copyArray z2
  z2''     <- copyArray z2
  pairing2 ("p1e1p2e2_run" ++ name) keepOnlyNLastIterations (parametersGameRematching ! "p1e1p2e2") exportConfigGameRematching (x1,y2)
  pairing2 ("p1e1p2e3_run" ++ name) keepOnlyNLastIterations (parametersGameRematching ! "p1e1p2e3") exportConfigGameRematching (x1',z2)
  pairing2 ("p1e2p2e1_run" ++ name) keepOnlyNLastIterations (parametersGameRematching ! "p1e2p2e1") exportConfigGameRematching (y1,x2)
  pairing2 ("p1e2p2e3_run" ++ name) keepOnlyNLastIterations (parametersGameRematching ! "p1e2p2e3") exportConfigGameRematching (y1',z2')
  pairing2 ("p1e3p2e1_run" ++ name) keepOnlyNLastIterations (parametersGameRematching ! "p1e3p2e1") exportConfigGameRematching (z1,x2')
  pairing2 ("p1e3p2e2_run" ++ name) keepOnlyNLastIterations (parametersGameRematching ! "p1e3p2e2") exportConfigGameRematching (z1',y2')
  pairing2 ("p1e1p2e1_run" ++ name) keepOnlyNLastIterations (parametersGameRematching ! "p1e1p2e1") exportConfigGameRematching (x1'',x2'')
  pairing2 ("p1e2p2e2_run" ++ name) keepOnlyNLastIterations (parametersGameRematching ! "p1e2p2e2") exportConfigGameRematching (y1'',y2'')
  pairing2 ("p1e3p2e3_run" ++ name) keepOnlyNLastIterations (parametersGameRematching ! "p1e3p2e3") exportConfigGameRematching (z1'',z2'')
