...
newtype Action = Action Bool  deriving (Show, Eq, Ord, NFData)
...
initialEnv1 = initialArray >>= \arr -> pure $ Env "Player1" 1 arr  0  0.2  (Rand.mkStdGen 3) (fmap (fmap toIdx)(Memory.fromSV(SV.replicate initialObservation))) (5 * 0.999) "NothingHappendYet" 0 (fmap (fmap toIdx) (Memory.fromSV(SV.replicate initialObservation)))
initialEnv2 :: M (Env N Observation Action)
initialEnv2 = initialArray >>= \arr ->  pure $ Env "Player2" 2 arr 0  0.2  (Rand.mkStdGen 100) (fmap (fmap toIdx)(Memory.fromSV(SV.replicate initialObservation))) (5 * 0.999) "NothingHappenedYet" 0 (fmap (fmap toIdx) (Memory.fromSV(SV.replicate initialObservation)))
...
toObs :: M (Action,Env N Observation Action) -> M Action -> M ((), (Observation Action,Observation Action))
toObs a1 a2 = do
             (act1,env1) <- a1
             act2 <- a2
...
toObsFromLS :: List '[M (Action,Env N Observation Action), M Action] -> M ((), (Observation Action,Observation Action))
toObsFromLS (x ::- (y ::- Nil))= toObs x y


-- From the outputted list of strategies, derive the context
fromEvalToContext :: List '[M (Action,Env N Observation Action), M Action] ->
