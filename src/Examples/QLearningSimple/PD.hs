...
toObs :: Monad m => m (Action,State N Observation Action) -> m (Action,State N Observation Action) -> m ((), ())
toObs a1 a2 = do
             (act1,env1) <- a1
             (act2,env2) <- a2
...
toObsFromLS :: Monad m => List '[m (Action,State N Observation Action), m (Action,State N Observation Action)] -> m ((), ())
toObsFromLS (x ::- (y ::- Nil))= toObs x y


-- From the outputted list of strategies, derive the context
fromEvalToContext :: Monad m =>  List '[m (Action,State N Observation Action), m (Action,State N Observation Action)] ->
                     MonadContext m () () (Action,Action) ()
...
stageDeterministic = [opengame|
   ...
   inputs    :      ;
   feedback  :      ;
   operation : pureDecisionQStage actionSpace "Player2" chooseBoltzQTable chooseUpdateBoltzQTable ;
   outputs   :  act2 ;
...
evalStage ::
     List '[ IO (Action, State N Observation Action), IO (Action, State N Observation Action)]
  -> MonadContext IO () () (Action, Action) ()
  -> List '[ IO (Action, State N Observation Action), IO (Action, State N Observation Action)]
...
hoist ::
     Applicative f
  => List '[ (Action, State N Observation Action), (Action, State N Observation Action)]
  -> List '[ f (Action, State N Observation Action), f (Action, State N Observation Action)]
hoist (x ::- y ::- Nil) = pure x ::- pure y ::- Nil

sequenceL ::
     Monad f
  => List '[ f (Action, State N Observation Action), f (Action, State N Observation Action)]
  -> f (List '[ (Action, State N Observation Action), (Action, State N Observation Action)])
...
evalStageM ::
     List '[ (Action, State N Observation Action), (Action, State N Observation Action)]
  -> Int
  -> IO [List '[ (Action, State N Observation Action), (Action, State N Observation Action)]]
