...
import qualified Examples.QLearning.OwnPriceObservation3Phases as Scenario

-- This experiment considers asymmetric cost runs where players only observe their own price but not the price of their opponent
...
parametersGameRematchingMap explore1 explore2 = fromList [ (("e1","e2"),p1e1p2e2)
                                       , (("e1","e3"),p1e1p2e3)
                                       , (("e1","e4"),p1e1p2e4)
                                       , (("e1","e5"),p1e1p2e5)
                                       , (("e1","e6"),p1e1p2e6)
...
exportConfigGameRematchingPhase2 name parameters arr1 arr2 obs = ExportAsymmetricLearners.ExportConfig
    ...
    -- ^ Stopping criterion: After how many runs should the computation be stopped?
    , mapStagesM_ = Scenario.mapStagesMFinalResult Scenario.configQL Scenario.configQLNoLearning parameters
